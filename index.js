    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'FM ABDULLAH'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://postimg.cc/xXwrBwtn'; // Default image
  }
}
...
async function connectToWA() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      const imageUrl = 'https://postimg.cc/xXwrBwtn';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923124001592@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
